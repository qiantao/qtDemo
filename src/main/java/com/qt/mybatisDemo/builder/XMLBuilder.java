...
//        }
    ...
    private static void insertSelective(StringBuffer buf) {
        ...
        for (ColumnInfo columnInfo : columnInfoList) {
            if ("create_time".equals(columnInfo.getColumnName())
                    || "modify_time".equals(columnInfo.getColumnName())
                || "id".equals(columnInfo.getColumnName())
                    ||!"".equals(columnInfo.getPrimaryKey()))
                continue;
            if (columnInfo.getColumnDefault() != null) {
                ...
                buf.append("                ");
                buf.append(columnInfo.getColumnName()).append(",\n");
                buf.append("            </if>\n");
            } else {
                buf.append("            ");
                buf.append(columnInfo.getColumnName()).append(",\n");
...
//                buf.append("            #{").append(columnInfo.getColumnName()).append(",jdbcType=");
                buf.append("            #{").append(BuildEntityStr.axaToaxA(columnInfo.getColumnName())).append(",jdbcType=");
                buf.append(JDBCTypeEnum.getJDBCTypeByDBType(columnInfo.getDataType())).append("},\n");
            }
        }
...
//                        .append("\" property=\"").append(columnInfo.getColumnName()).append("\" />\n");
...
//        }
    ...
    private static void update(StringBuffer buf) {
        ...
        ColumnInfo groupCol = null;
        for (ColumnInfo columnInfo : columnInfoList) {
//            if ("group_id".equals(columnInfo.getColumnName())) {
//                groupCol = columnInfo;
//                continue;
//            }
            ...
            if ("create_time".equals(columnInfo.getColumnName())
                    || "modify_time".equals(columnInfo.getColumnName())
                    || "create_user".equals(columnInfo.getColumnName())
                    || "company_id".equals(columnInfo.getColumnName())) continue;
        ...
        buf.setLength(buf.length() - 2);
        buf.append("\n        where " + temp.getColumnName() + " = #{" + BuildEntityStr.axaToaxA(temp.getColumnName()) + ",jdbcType=").append(JDBCTypeEnum.getJDBCTypeByDBType(temp.getDataType())).append("}\n");
//        if (groupCol != null) {
//            buf.append("\n        and " + groupCol.getColumnName() + " = #{" + BuildEntityStr.axaToaxA(groupCol.getColumnName()) + ",jdbcType=").append(JDBCTypeEnum.getJDBCTypeByDBType(temp.getDataType())).append("}\n");
//        }
        buf.append("    </update>\n\n");
        //update ---结束
    }
...
//            }
        ...
        buf.setLength(buf.length() - 2);
        buf.append("\n           where " + temp.getColumnName() + " = #{item." + BuildEntityStr.axaToaxA(temp.getColumnName()) + ",jdbcType=").append(JDBCTypeEnum.getJDBCTypeByDBType(temp.getDataType())).append("}\n");
//        if (groupCol != null) {
//            buf.append("\n        and " + groupCol.getColumnName() + " = #{item." + BuildEntityStr.axaToaxA(groupCol.getColumnName()) + ",jdbcType=").append(JDBCTypeEnum.getJDBCTypeByDBType(temp.getDataType())).append("}\n");
//        }
        buf.append("        </foreach>\n");
        buf.append("    </update>\n\n");
        //update ---结束
...
//            } else {
                buf.append("            ").append(columnInfo.getColumnName()).append(" = #{").append(BuildEntityStr.axaToaxA(columnInfo.getColumnName()));
//                buf.append("            ").append(columnInfo.getColumnName()).append(" = #{").append(columnInfo.getColumnName());
                buf.append(",jdbcType=").append(JDBCTypeEnum.getJDBCTypeByDBType(columnInfo.getDataType())).append("},\n");
//            }

        }
        buf.append("        </trim>\n");
//        buf.append("        where " + temp.getColumnName() + " = #{" + temp.getColumnName() + ",jdbcType=").append(JDBCTypeEnum.getJDBCTypeByDBType(temp.getDataType())).append("}\n");
...
//        }
    ...
    private static void updateSelectiveList(StringBuffer buf) {
        buf.append("    <!-- 选择更新-->\n");
        buf.append("    <update id=\"").append(MethodEnum.UPDATE_SELECTIVE_LIST.getValue()).append("\" parameterType=\"").append(entityPath).append("\">\n");
        buf.append("        <foreach collection=\"list\" item=\"item\" separator=\";\">\n");
        buf.append("        update \n");
        buf.append("            <include refid=\"TableName\" />\n");
        buf.append("            <trim prefix=\"set\" suffixOverrides=\",\"> \n");
